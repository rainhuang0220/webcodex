...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
    ...
    require_all_scopes(
        model_spec(
            ...
            "Atomically claim execution ownership for the explicit current AgentTask assignee. Creates one leased fenced AgentTaskAttempt only; it does not dispatch CodingAgentRun, Job, shell, model, Wake, Endpoint, or Workflow Session work. Exact keyed retry returns the original Attempt, fence, and attempt_ref.",
        ),
        COMMUNICATION_MANAGE_SCOPES,
    ),
    require_all_scopes(
        permission_risk(
            model_spec(
                ...
                "Select the Agent Endpoint continuation backend for the exact latest unexpired fenced AgentTaskAttempt. Pass attempt_ref or the exact task, attempt, assignee, fence, and controller generation. This call never chooses an Endpoint: it durably creates/replays one Task-origin Wake with a nullable carrier, then the existing event-driven continuation controller dispatches only when a current wake-capable Endpoint later claims it. TaskAttempt and Endpoint leases remain independent; no synthetic Conversation Message or Inbox Delivery is created. The ref is not authority. Heartbeat, completion, and coding-run still take the explicit tuple.",
            ),
            PERMISSION_RISK_WRITE,
        ),
        COMMUNICATION_MANAGE_SCOPES,
