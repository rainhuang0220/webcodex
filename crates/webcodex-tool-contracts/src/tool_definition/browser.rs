    ...
    require_any_scopes(
        permission_risk(
            model_spec(
                ...
                "Effectful Browser gateway for launch, new_page, navigate, reload, click, input_text, select_option, set_value, upload_file, key, clear_diagnostics, close_page, and close_browser. Element effects run only when the current snapshot lists that action. Authority is checked before dispatch: launch needs browser:launch, upload_file needs browser:control plus project:read, and other effects need browser:control. Successful page/control effects include bounded post-effect stability from document/DOM and network quiet signals; stability=false is observational and never makes a completed effect retry-safe. Reload/navigation stale prior element ids; clear_diagnostics uses a CDP barrier before reset. Uploads accept one same-Runner project-relative regular file. Effects preserve not_started/completed/outcome_unknown certainty and never blindly retry uncertain actions. No arbitrary protocol, executable, profile, remote endpoint, or script input is accepted.",
            ),
